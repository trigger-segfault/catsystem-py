...
def registerimg(arctype) -> NoReturn:
    if arctype in _registered_imgs:
        raise ValueError('Archive type {0.__name__!r} already registered'.format(arctype))
    _registered_imgs.append(arctype)
...
def identifyimg(file:Union[io.BufferedReader,str,bytes], filename:str=None, minscore:int=1) -> Tuple[type, int]:
    ...
    for arctype in _registered_imgs:
        arcscore = 0
        #
...
def openimg(file:Union[io.BufferedReader,str,bytes], filename:str=None, **kwargs) -> Archive:
    if isinstance(file, str):
        if filename is None:
            filename = file
        with open(file, 'rb') as file:
    ...
    #
    arctype, arcscore = identifyimg(file, filename)
    if arctype is None:
...
class ImageFrame(object):
    """ImageFrame() -> empty archive entry
    ImageFrame(name, offset, length, *, [archive=archive]) -> archive entry
    """
    #
    # INSTANCE METHODS:
    #
    def __init__(self, fullname:str=None, width:int=0, height:int=0, offset:int=0, length:int=0, *, archive:Archive=None):
