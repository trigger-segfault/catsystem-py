...
"""
import catsys, struct, io, os, zlib
catsys.SceneScript
cs01 = catsys.SceneScript(r"data\games\gris1utf8\scene\01_neko3.cst")
cs01.write(r"data\games\gris1utf8\scene\01_neko3_out.cst", autofill=True)
cs01in = catsys.SceneScript(r"data\games\gris1utf8\scene\01_neko3_out.cst")

csl01 = catsys.SceneLocalization(r"data\games\gris1utf8\scene\01_neko3.cstl")
csl01.write(r"data\games\gris1utf8\scene\01_neko3_out.cstl", autofill=True)
csl01in = catsys.SceneLocalization(r"data\games\gris1utf8\scene\01_neko3_out.cstl")

def tuplemessage(locmsg):
    return tuple((msg.name, msg.message) for msg in locmsg.messages)

def tuplemessages(cstlmsgs):
    return tuple(tuplemessage(locmsg) for locmsg in cstlmsgs)

csr01 = catsys.tr.SceneTranslation(cs01, None)
"""

"""

def getmessage(index, lang, orig_line):
    name = ''
    message = ''
    for line in orig_line:
        if line.kind == catsys.SceneLineType.NAME:
            name += line.content
        elif line.kind == catsys.SceneLineType.MESSAGE:
            ...
            if content:
                if message:
                    message += '\n'
                message += content
    return catsys.SceneMessage(index, lang, name, message)

def makecstl(cst):
    if not isinstance(cst, catsys.SceneScript):
        cst = catsys.SceneScript(cst)
    cstr = catsys.tr.SceneTranslation(cst)
    cstl = catsys.SceneLocalization()
    cstl.languages = [catsys.SceneLanguage('en', 0), catsys.SceneLanguage('jp', 1)]
    #cstl.messages = [catsys.SceneLocalizationMessage(i, cstl.languages, [catsys.SceneMessage(i, cstl.languages[j], m[0].name, m[0].message) for j in range(2)]) for i,m in enumerate(cstr.orig_lines[:-1])]
    cstl.messages = [catsys.SceneLocalizationMessage(i, cstl.languages, [getmessage(i, cstl.languages[j], orig_line) for j in range(2)]) for i,orig_line in enumerate(cstr.orig_lines[:-1])]
    return cstl

copl.messages = [catsys.SceneLocalizationMessage(i, copl.languages, [catsys.SceneMessage(i, copl.languages[j], m[0].name, m[0].message) for j in range(2)]) for i,m in enumerate(copr.orig_lines[:-1])]

copl.messages = [catsys.SceneLocalizationMessage(i, copl.languages, [getmessage(i, copl.languages[j], orig_line) for j in range(2)]) for i,orig_line in enumerate(copr.orig_lines[:-1])]
"""
