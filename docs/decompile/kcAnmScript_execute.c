...
//Used to strip offset prefixes to see if it fixes linter
//REGEX: ^  \/\*[0-9a-f]+(,[0-9a-f]*)?\*\/ *
//REPLACE: 
...

{
  ...
  ANM_SCRIPT *unaff_ESI; //undefined4 *unaff_ESI; // ANM_SCRIPT*
  undefined4 *puVar2;
  
  iVar1 = (INT)&unaff_ESI->Timelines[0]; // unaff_ESI[1];
  if (iVar1 != NULL) {
    if (*(INT *)(iVar1 + -4) < 2) {
    ...
    // unaff_ESI[1] = 0;
    unaff_ESI->Timelines = (ANM_TIMELINE *)NULL;
  }

  unaff_ESI->TimelineCount = 0; // *unaff_ESI = 0;
  unaff_ESI->Unk2 = 0;          // unaff_ESI[2] = 0;
  unaff_ESI->Unk3 = 0;          // unaff_ESI[3] = 0;
  unaff_ESI->Counter = 0;       // unaff_ESI[4] = 0;
  unaff_ESI->Wait = 0;          // unaff_ESI[6] = 0;
  iVar1 = 0x40;
  puVar2 = unaff_ESI + 8;
  while (iVar1 != 0) {
    iVar1 += -1;
  ...
  *(INT *)(*(INT *)(iVar1 + 0x18) + 8) = *(INT *)(iVar1 + 0x18);
  ...
  // unaff_ESI[0x4d] = 0;
  // unaff_ESI[0x4e] = 0;
  unaff_ESI->FrameID = 0; // unaff_ESI[0x4a] = 0;
  unaff_ESI->Blend = 255; // unaff_ESI[0x4b] = 0xff;
  unaff_ESI->Disp = TRUE; // unaff_ESI[0x4c] = 1;
  unaff_ESI->PosX = 0; // unaff_ESI[0x4d] = 0;
  unaff_ESI->PosY = 0; // unaff_ESI[0x4e] = 0;
...

{
  ...
  if (fileData->Hdr.Magic != MAGIC_ANM) { // "ANM"
    // "kcAnmScript : データ形式が違います"
    // ("kcAnmScript: Data format is different")
    kclib_logError(LOG_kcAnmScript_different_data_format);
    return FALSE;
  }
  if (fileData->Hdr.TimelineCount == 0) {
    // "kcAnmScript : タイムライン数が不正です"
    // ("kcAnmScript: Timeline count is invalid")
    kclib_logError(LOG_kcAnmScript_invalid_timeline_count);
    return FALSE;
  }
  //if (fileSize < fileData->Hdr.TimelineCount * 0x44 + 0x20U) {
  if (fileSize < fileData->Hdr.TimelineCount * sizeof(ANM_TIMELINE) + sizeof(ANM_FILEHEADER)) { // * 0x44 + 0x20U
    // "kcAnmScript : データサイズが不正です"
    // ("kcAnmScript: Data size is invalid")
    kclib_logError(LOG_kcAnmScript_invalid_data_size);
    return FALSE;
  }
  anmScr->Unk2 = fileData->Hdr.AnmUnk1; // ANM.[....]tcnt .... ....
  anmScr->TimelineCount = fileData->Hdr.TimelineCount; // ANM. ....[tcnt].... ....
  iVar1 = (ANM_TIMELINE *)cs2_HeapAlloc_logged_004b02b0(0, fileData->Hdr.TimelineCount * sizeof(ANM_TIMELINE)); // * 0x44);
  anmScr->Timelines = iVar1;
  cs2_strmemcpy_004b1600(iVar1, &fileData->Timelines[0], fileData->Hdr.TimelineCount * sizeof(ANM_TIMELINE)); // * 0x44);
